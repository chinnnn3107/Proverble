...
class ResultScreen(Screen):
    ...
    def render(self):
        ...
        # ---------------- Answer ----------------
        label_text = "Answer"
        label_surf = self.font_text.render(label_text, True, (220, 220, 220))
        label_rect = label_surf.get_rect(center=(self.W // 2, self.H // 2 - 30))

        ans_text = self.answer
        ans_surf = self.font_text.render(ans_text, True, (0, 0, 0))  
        ans_rect = ans_surf.get_rect(midtop=(self.W // 2, label_rect.bottom + 20))  

        padding_x, padding_y = 20, 8
        bg_rect = pygame.Rect(
            ans_rect.left - padding_x,
            ans_rect.top - padding_y,
            ans_rect.width + 2 * padding_x,
            ans_rect.height + 2 * padding_y,
        )

        pygame.draw.rect(self.surface, (255, 153, 255), bg_rect, border_radius = 10)  
        pygame.draw.rect(self.surface, (0, 0, 0), bg_rect, 2, border_radius = 10)   

        self.surface.blit(label_surf, label_rect)
        self.surface.blit(ans_surf, ans_rect)
        ...
        # ---------------- Draw Buttons ----------------
        draw_button(self.button_restart, (34, 34, 34), "Restart")
        draw_button(self.button_quit, (34, 34, 34), "Quit")
